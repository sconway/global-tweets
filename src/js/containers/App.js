...
import { tweetList } from "../constants/tweetList"
...
class App extends Component {
  ...
  initFeed() {
    ...
    // this.timer = setInterval(self.addTweet.bind(self), 500);
    clearTimeout(this.tweetTimeout);
    this.root.add(this.points);
    this.addTweet();
  }
  ...
  addTweet() {
    ...
    if (tweets && tweets.length > 0 && !this.isCountryClicked) {
      ...
      const color = getColor(tweet.sentiment);
      ...
      if (this.props.numTweets > 50) this.points.children.shift();

      this.tweetTimeout = setTimeout(this.addTweet, 1000);
    }
  }
  ...
  async loadMap() {
    ...
    try {
      worldJson = await json("/json/world.json")
    } catch (error) {
      console.log("JSON error: ", error)
    }
    // Setup cache for country textures
    this.countries = topojson.feature(worldJson, worldJson.objects.countries);
    ...
    setEvents(
      ...
      5,
      this.TWEEN,
      self.onPointHover.bind(self),
      self.onPointHoverOff.bind(self)
    );
  
    try {
      const countriesJson = await json("/json/countries.json")
  ...
  onReady() {
    ...
    growObject(this.TWEEN, this.root, 1, 2000, 500, this.initFeed);
  }
  ...
  onCountryHoverOff() {
    if (!this.isCountryClicked) {
      ...
      if (!this.props.isPointHovered) this.toggleGlobeVisibility(0, 0.99, 1);
    }
  }
  ...
  onGlobeMouseUp(event) {
    ...
    if (!this.isCountryClicked && !this.props.isPointHovered && isStatic) {
      ...
      const { x, y, z } = convertToXYZ(latlng, 900);
   ...
   */
  update() {
    if (controls && !this.props.isPointHovered) controls.update();
  ...
  render() {
    return (
      <main className="body">
        ...
        />
        <TweetCount numTweets={this.props.numTweets} />
        <TweetBox
